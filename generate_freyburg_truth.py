...
def setup_mf6_freyberg(testpest=True):
    import numpy as np
    import pandas as pd
    ...
    shutil.copy2(os.path.join(exe_dir, mf6_exe), os.path.join(tmp_model_ws, mf6_exe))

    # shutil.copy2(os.path.join(exe_dir, mf6_exe), os.path.join(template_ws, mf6_exe))
    shutil.copy2(os.path.join(exe_dir, pest_exe), os.path.join(tmp_model_ws, pest_exe))

    # run mf6 simulation
    pyemu.os_utils.run(mf6_exe,cwd=tmp_model_ws)
    ...
    # test forward run
    if testpest:
        pyemu.os_utils.run("pestpp-ies freyberg.pst", cwd=template_ws, verbose=True)
